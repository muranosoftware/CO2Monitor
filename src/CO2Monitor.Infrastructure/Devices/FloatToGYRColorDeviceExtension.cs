...
namespace CO2Monitor.Infrastructure.Devices {
	public class FloatToGyrColorDeviceExtension : IDeviceExtension {
		...
		public FloatToGyrColorDeviceExtension(string field, IDevice device) {
			if (device == null)
			...
			CreateFieldAndActionDictionariesAndEventsList(field);
		}
		...
		public DeviceInfo BaseInfo {
			get => new DeviceInfo(_fields.Keys, _actions.Keys, _events);
			set { }
		}
		...
		public string State {
			get {
				...
				foreach (KeyValuePair<DeviceStateFieldDeclaration, Func<Variant>> field in _fields)
					json.Add(new JProperty(field.Key.Name, field.Value().String));
		...
		private void CreateFieldAndActionDictionariesAndEventsList(string field) {
			...
			_actions = new Dictionary<DeviceActionDeclaration, Action<Variant>> {
				{
					new DeviceActionDeclaration(field + "SetRedLevel", VariantDeclaration.Float), (val) => {
				...
				{
					new DeviceActionDeclaration(field + "SetYellowLevel", VariantDeclaration.Float), (val) => {
						if (val.Declaration != VariantDeclaration.Float)
							throw new CO2MonitorArgumentException();
		...
		private GyrColor FloatToGyrColor(double val) {
			if (val < YellowLevel)
				return GyrColor.Green;
			if (val >= RedLevel)
				return GyrColor.Red;
			return GyrColor.Yellow;
		}
	}
}
